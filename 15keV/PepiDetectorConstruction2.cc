...
//=========================================
  ...
  density1 = 3.21*g/cm3;//4.93*g/cm3;// 3.21*g/cm3;
  G4Material* SiC = new G4Material(name="SandP",density1,ncomponents=2);
  SiC ->AddMaterial(Silicon, fractionmass=50*perCent);//fractionmass=70.05*perCent
...
//      G4double y1 = - i*fPixelSize2X + fPixiRadSize2X/2;// - fPixelSizeY/2;
  ...
  std::vector<G4ThreeVector> coordinates = ReadCoordinates("../data/125_um_Coord.txt");
...
fSphere6Logical = new G4LogicalVolume(fSphere6Solid, 
 ...
 std::vector<G4ThreeVector> coordinates3 = ReadCoordinates("../data/125_um_Coord3.txt");
 G4int Numb_speck3 = 112000;
...
fSphere7Logical = new G4LogicalVolume(fSphere7Solid, 
 ...
 // Build the Speckel's Envelope 
  ...
  std::vector<G4ThreeVector> coordinates4 = ReadCoordinates("../data/125_um_Coord4.txt");
 G4int Numb_speck4 = 112000;
...
// -----------------NYLON FIBER----------------------------------------------------------------
/*
 fObject3Solid = new G4Tubs("tube3",
                              0.0*mm,                         //its R_min
                              0.03/2*mm,   			     // its R_max
                              1*cm,		            //its symetric angle azim
                              0*deg,                     // its final angle 
                              360*deg);                       //its symetric zenith angle)//its final angle zenith               //its half height
...
fSphere8Logical = new G4LogicalVolume(fSphere8Solid, 
                                      fSubMaterial,
                                      "SphereLV8");
G4ThreeVector objectPositionSph8 = G4ThreeVector(0*mm, 0*mm, 0);//-3.52*mm
